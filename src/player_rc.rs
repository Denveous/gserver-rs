...
impl Player {
    pub async fn preserve_admin_only_server_options(options: &str, server_arc: Arc<RwLock<Server>>) -> String {
        let mut out = String::new();
        let options = options.replace("\r\n", "\n");
        let had_trailing_newline = options.ends_with('\n');
        let mut lines: Vec<&str> = options.split('\n').collect();
        if had_trailing_newline && !lines.is_empty() && lines.last() == Some(&"") {
            lines.pop();
        }
        
        let srv = server_arc.read().await;
        let admin_only = [
        ...
        for (i, line) in lines.iter().enumerate() {
            if let Some(eq_pos) = line.find('=') {
                ...
                if admin_only.contains(&name) {
                    let val = srv.settings.get_or(name, "");
                    let new_line = format!("{} = {}", name, val);
            ...
            if i < lines.len() - 1 || had_trailing_newline {
                out.push('\n');
            }
        }
    ...
    pub async fn msg_pli_rc_serveroptionsget(&mut self, _packet: &[u8]) -> bool {
        ...
        let mut settings_str = String::new();
        {
            ...
            if let Ok(data) = srv.fs.load_file("config/serveroptions.txt") {
                settings_str = String::from_utf8_lossy(&data).into_owned();
            } else {
                for (k, v) in srv.settings.get_all() {
                    settings_str.push_str(&format!("{}={}\n", k, v));
                }
            }
        }
        
        let mut buf = Buffer::new();
        buf.write_byte(PLO_RC_SERVEROPTIONSGET);
        buf.write_bytes(gtokenize_text(&settings_str).as_bytes());
        self.send(&buf.bytes()).await;
        true
    }

    pub async fn msg_pli_rc_serveroptionsset(&mut self, packet: &[u8]) -> bool {
        if self.player_type != PLTYPE_RC && self.player_type != PLTYPE_RC2 && (self.player_type & PLTYPE_ANYRC) == 0 {
            return true;
        }
        if !self.has_right(PLPERM_SETSERVEROPTIONS) {
            let chat_msg = format!("Server: {} is not authorized to change the server options.", self.account_name);
            let mut buf = Buffer::new();
            buf.write_byte(PLO_RC_CHAT);
            buf.write_bytes(chat_msg.as_bytes());
            self.send(&buf.bytes()).await;
            return true;
        }
        ...
        if packet.len() > 1 {
            options = guntokenize_text(&String::from_utf8_lossy(&packet[1..]));
        }
        
        if !self.has_right(PLPERM_MODIFYSTAFFACCOUNT) {
            options = Self::preserve_admin_only_server_options(&options, self.server.clone()).await;
        }
        
        {
            let mut srv = self.server.write().await;
            let _ = srv.settings.load_from_string(&options);
            if !options.ends_with('\n') {
                options.push('\n');
            }
            let _ = srv.fs.save_file("config/serveroptions.txt", options.as_bytes());
            srv.load_settings();
        }
        
        let srv = self.server.read().await;
        srv.send_rc_chat(&format!("{} has updated the server options.", self.account_name)).await;
        true
    }

    pub async fn msg_pli_rc_folderconfigget(&mut self, _packet: &[u8]) -> bool {
        if self.player_type != PLTYPE_RC && self.player_type != PLTYPE_RC2 && (self.player_type & PLTYPE_ANYRC) == 0 {
            return true;
        }
        
        let mut folders_config = String::new();
        {
            let srv = self.server.read().await;
            if let Ok(data) = srv.fs.load_file("config/foldersconfig.txt") {
                folders_config = String::from_utf8_lossy(&data).into_owned();
            }
        }
        
        folders_config = folders_config.replace("\r\n", "\n").replace("\r", "\n");
        let mut buf = Buffer::new();
        buf.write_byte(PLO_RC_FOLDERCONFIGGET);
        buf.write_bytes(gtokenize_text(&folders_config).as_bytes());
        self.send(&buf.bytes()).await;
        true
    }

    pub async fn msg_pli_rc_folderconfigset(&mut self, packet: &[u8]) -> bool {
        if self.player_type != PLTYPE_RC && self.player_type != PLTYPE_RC2 && (self.player_type & PLTYPE_ANYRC) == 0 {
            return true;
        }
        if !self.has_right(PLPERM_SETFOLDEROPTIONS) {
            let chat_msg = format!("Server: {} is not authorized to change the folder config.", self.account_name);
            let mut buf = Buffer::new();
            buf.write_byte(PLO_RC_CHAT);
            buf.write_bytes(chat_msg.as_bytes());
            self.send(&buf.bytes()).await;
        ...
        if packet.len() > 1 {
            folders = guntokenize_text(&String::from_utf8_lossy(&packet[1..]));
        }
        folders = folders.replace("\\", "");
        folders = folders.replace("\n", "\r\n");
        
        {
            let srv = self.server.write().await;
            let _ = srv.fs.save_file("config/foldersconfig.txt", folders.as_bytes());
        }
        
        let srv = self.server.read().await;
        srv.send_rc_chat(&format!("{} updated the folder config.", self.account_name)).await;
        true
    }

    pub async fn msg_pli_rc_respawnset(&mut self, _packet: &[u8]) -> bool {
        true
    }

    pub async fn msg_pli_rc_horselifeset(&mut self, _packet: &[u8]) -> bool {
        true
    }

    pub async fn msg_pli_rc_apincrementset(&mut self, _packet: &[u8]) -> bool {
        true
    }

    pub async fn msg_pli_rc_baddyrespawnset(&mut self, _packet: &[u8]) -> bool {
        true
    }

    pub async fn msg_pli_rc_playerpropsget(&mut self, _packet: &[u8]) -> bool {
        true
    }

    pub async fn msg_pli_rc_playerpropsset(&mut self, packet: &[u8]) -> bool {
        ...
        let srv = self.server.read().await;
        let target_arc = srv.players.get(&target_id).cloned();
        drop(srv);
        
        if let Some(target_arc) = target_arc {
            let mut target = target_arc.write().await;
            
            let self_is_target = target.account_name == self.account_name;
            let has_other_perm = self.has_right(PLPERM_SETATTRIBUTES);
            let has_self_perm = self.has_right(PLPERM_SETSELFATTRIBUTES);
            
            if (!self_is_target && !has_other_perm) || (self_is_target && !has_self_perm) {
                let chat_msg = format!("Server: {} is not authorized to set the properties of {}", self.account_name, target.account_name);
                let mut chat_buf = Buffer::new();
                chat_buf.write_byte(PLO_RC_CHAT);
                chat_buf.write_bytes(chat_msg.as_bytes());
                self.send(&chat_buf.bytes()).await;
                return true;
            }
            
            // In Go: targetPlayer.setPropsFromRC(buf, p)
            // TODO: fully implement target.set_props_from_rc(&mut buf, self);
            // target.save_account();
    ...
    pub async fn msg_pli_rc_disconnectplayer(&mut self, packet: &[u8]) -> bool {
        if packet.len() < 3 {
            return true;
        }
        let mut buf = Buffer::from_bytes(packet[1..].to_vec());
        ...
        if !self.has_right(PLPERM_DISCONNECT) {
            let chat_msg = format!("Server: {} is not authorized to disconnect players.", self.account_name);
            let mut chat_buf = Buffer::new();
            chat_buf.write_byte(PLO_RC_CHAT);
            chat_buf.write_bytes(chat_msg.as_bytes());
            self.send(&chat_buf.bytes()).await;
        ...
        let reason = buf.read_gstring();
        let mut disconnect_message = format!("One of the server administrators, {}, has disconnected you", self.account_name);
        
        let target_account_name = target_arc.read().await.account_name.clone();
        
        if !reason.is_empty() {
            disconnect_message.push_str(&format!(" for the following reason: {}", reason));
        } else {
            disconnect_message.push('.');
        }
        
        {
            let mut target = target_arc.write().await;
            
            let mut disc_buf = Buffer::new();
            disc_buf.write_byte(PLO_DISCMESSAGE);
            disc_buf.write_byte(0); 
            target.send(disc_buf.bytes()).await;
            
            let mut str_buf = Buffer::new();
            str_buf.write_string8(&disconnect_message);
            target.send(str_buf.bytes()).await;
            
            target.tx = None;
        }
        
        let srv = self.server.read().await;
        srv.send_rc_chat(&format!("{} disconnected {}", self.account_name, target_account_name)).await;
...
fn read_rc_account_payload(packet: &[u8], packet_id: u8) -> String {
    ...
    let mut buf = Buffer::from_bytes(payload.to_vec());
...
impl Player {
    ...
    pub async fn msg_pli_rc_adminmessage(&mut self, packet: &[u8]) -> bool {
        ...
        if packet.len() > 1 {
            ...
            buf2.write_string8(&format!("Admin {}:\u{A7}{}", self.account_name, message));
